...
void Chunk::generate_chunk(World* world) {
    destroy();
        ...
        for(int bz = 0; bz < CHUNK_SIZE; bz++) {
            for(int by = 0; by < CHUNK_SIZE; by++) {
                blocks[bx][by][bz].type =
                    world->generation.generate_block(position.x, position.y, position.z, bx, by, bz);
            }
        }
    }
}

void Chunk::update_area(Chunk* (&chunks)[LOAD_DISTANCE][WORLD_HEIGHT][LOAD_DISTANCE]) {
    update_mesh();
...
bool Chunk::chk_block(int ix, int iy, int iz) {
    if(ix > CHUNK_SIZE - 1) {
        // Left adjacent chunk
...
void Chunk::generate_side(BlockType block_type, int sx, int sy, int sz, glm::vec3 up, glm::vec3 right, bool chkFlip, bool flip) {
...
void Chunk::save(const char* root) {
...
void Chunk::load(const char* root) {
    char* fname = (char*)malloc(100);
    sprintf(fname, "%s/chunks/%i %i %i.chunk", root, position.x, position.y, position.z);
    ...
    if(!xz) { // X axis
        ...
        for(int x = 0; x < LOAD_DISTANCE; x++) {
            for(int y = 0; y < WORLD_HEIGHT; y++) {
                wall_chunks[x][y]->position = glm::ivec3(x + 1, y, 0.0f);
                wall_chunks[x][y]->generate_chunk(world);
            }
        }
    }
}
